...
public class Register extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out = response.getWriter();
		         // System.out.println(name + mail + pass + "         szxdcfghbjndmkllachscvausckahscvacsbjkcbjhcsvajcvjkahvahc");
		Customer cust = new Customer(name,mail,pass);
//                System.out.println(cust.getUserName() + cust.getEmailId()+ cust.getPassword() + "    scvschvckhsvckjsvchsvc");
		try
		{
			db.addCustomer(cust);
		...
		{
                    e.printStackTrace();
                    out.println("<html><script>alert("+ e.getMessage() +");window.location.href='Register';</script></html>");
		}
	}
